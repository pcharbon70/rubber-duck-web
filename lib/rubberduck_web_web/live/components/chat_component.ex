defmodule RubberduckWebWeb.CollaborativeCodingLive.ChatComponent do
  ...
  def handle_event("send_message", %{"message" => %{"content" => content}}, socket)
    ...
    send(
      self(),
      {:chat_message,
       %{content: content, user_id: socket.assigns.user.id, session_id: socket.assigns.session_id}}
    )
  ...
  def render(assigns) do
    ...
    <!-- System Broadcast Area (Top 20%) -->
          <div
            class="system-broadcast-area flex-none bg-base-50 border-b border-base-300"
    ...
    <!-- Conversation Area (Bottom 80%) -->
          <div class="conversation-area flex-1 flex flex-col min-h-0">
            ...
            <div
              class="conversation-messages flex-1 overflow-y-auto p-4"
              id={"messages-#{@id}"}
            >
            ...
            <div class="message-input flex-none border-t border-base-300 p-4">
              <form phx-submit="send_message" phx-target={@myself} class="join w-full">
                <textarea
                  ...
                  disabled={@connection_state != :connected}
                ></textarea>
                ...
                <button
                  ...
                  disabled={
                    @connection_state != :connected or @loading or String.trim(@message_input) == ""
                  }
                >
                  <%= if @loading do %>
  ...
  defp get_sender_name(:user, _sender_id) do
